...
<<<<<<< HEAD
			...
			_streamId = streamId;
			std::cout << "--- 1st time -> stream ID has been set ---" << std::endl;
=======
			_streamId = streamId;
>>>>>>> 49b321f55b976b0ac9d86dfd52766685cb388d8e
		}
...
<<<<<<< HEAD
		std::cout << "--- update from middleware, new frame arrived arrived arrived ---" << std::endl;
=======
>>>>>>> 49b321f55b976b0ac9d86dfd52766685cb388d8e
	}
...
int main (int argc, char *argv[])
{
	...
	else
	{
		...
		xmlFiles.push_back(
			...
			//"file://home/hazel/rtivideo-gstreamer-1.0/rticonnextdds-usecases/VideoData02/ExampleCode/src/Config/base_profile_no_multicast.xml");
		xmlFiles.push_back(
	...
	try
	{
		...
		// video.
<<<<<<< HEAD
		std::cout << "--- vid framework for display created? ---" << std::endl;
		EMDSVideoOutput *vout = new EMDSVideoDisplayOutput();
		std::cout << "------EMDSVideoOutput data type:	" << typeid(((EMDSVideoDisplayOutput *)vout)).name() <<std::endl;
		//EMDSVideoOutput *vout = "appsrc name=\"src\" is-live=\"true\"do-timestamp=\"true\" caps=\"video/x-vp8, width=(int)640, height=(int)360 framerate=1000/1\" ! queue2 ! vp8dec ! ffmpegcolorspace ! videoconvert ! autovideosink";

		if (vout == NULL)
		{
			std::cout << "No Error, video not created" << std::endl;
		}
		if (vout != NULL)
		{
			std::cout << "--- Yes, seen as above!  ---" << std::endl;
=======
		EMDSVideoOutput *vout = new EMDSVideoDisplayOutput();
		if (vout == NULL)
		{
...
<<<<<<< HEAD
		...
		//videoMetadata = "appsrc name=\"src\" is-live=\"true\"do-timestamp=\"true\" caps=\"video/x-vp8, width=(int)640, height=(int)360 framerate=1000/1\" " ;
		//GetStreamMetadata() = videoMetadata;
		//GetStreamMetadata() = "";

		std::cout << "---Metadata  " << videoMetadata << std::endl;
		std::cout << "------metadata data type:	" << typeid(videoMetadata).name() <<std::endl;
=======
		std::string videoMetadata =
			((EMDSVideoDisplayOutput *)vout)->GetStreamMetadata();
